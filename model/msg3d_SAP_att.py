...
class Model(nn.Module):
    def __init__(self,
                 ...
                 num_g3d_scales,
                 graph,
                 in_channels=5,#keep same with numhead
                 soft_scale=20,
                 num_head =5):
        ...
        # r=3 STGC blocks
        self.gcn3d1 = MultiWindow_MS_G3D(num_head, c1, A_binary, num_g3d_scales, window_stride=1)
        self.sgcn1 = nn.Sequential(
            MS_GCN(num_gcn_scales, num_head, c1, A_binary, disentangled_agg=True),
            MS_TCN(c1, c1),
            MS_TCN(c1, c1))
        ...
        self.gen_angle = SAP(soft_scale=soft_scale, num_head=num_head)
        self.cbam = CBAM(num_head, 2, no_spatial=True)

    def forward(self, x):
        N, _, T, _, _ = x.size()
        x = x.permute(0, 2, 4, 3, 1).contiguous().view(N, T, -1)

        N,T,_ = x.shape
        x = self.gen_angle(x)
        N, C, T, V, M = x.size()
        x = x.permute(0, 4, 1, 2, 3).contiguous().view(-1, C, T, V)
        x = self.cbam(x)
