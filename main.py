...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help='if true,need freeze weights except ingnore_weights')
    parser.add_argument(
        '--unfreeze_epoch',
        type=int,
        default=0,
        help='begin unfreeze epoch')
    parser.add_argument(
        '--seed',
        type=int,
...
class Processor():
    ...
    def load_model(self):
        ...
        # self.loss = FocalLoss(class_num=60).cuda(output_device)#
    ...
    def train(self, epoch, save_model=False):
        if self.arg.pretrain:
            if epoch < self.arg.unfreeze_epoch:
                print("begin freeze>>>>>>>>>>>>>>>>>>>>>>>>")
                for name, p in self.model.named_parameters():
                    if(name not in self.arg.ignore_weights):
                        p.requires_grad = False
            else:
                for name, p in self.model.named_parameters():
                    if(name not in self.arg.ignore_weights):
                        p.requires_grad = True
                self.arg.pretrain = False
                print("begin unfreeze>>>>>>>>>>>>>>>>>>>>>>>>")
        self.model.train()
